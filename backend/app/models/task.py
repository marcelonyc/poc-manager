...
from sqlalchemy import Column, Integer, String, Boolean, DateTime, ForeignKey, Text, Enum as SQLEnum, Table, UniqueConstraint
...
task_group_tasks = Table(
    ...
    Column('task_group_id', Integer, ForeignKey('task_groups.id', ondelete='CASCADE'), nullable=False),
...
class Task(Base):
    ...
    resources = relationship("TaskTemplateResource", back_populates="task", cascade="all, delete-orphan")
    task_groups = relationship("TaskGroup", secondary=task_group_tasks, back_populates="tasks")
...
class TaskGroup(Base):
    ...
    tasks = relationship("Task", secondary=task_group_tasks, back_populates="task_groups")
    resources = relationship("TaskGroupResource", back_populates="task_group", cascade="all, delete-orphan")
...
class POCTask(Base):
    ...
    task_id = Column(Integer, ForeignKey("tasks.id"), nullable=True)  # Can be custom
    ...
    task_criteria = relationship("TaskSuccessCriteria", back_populates="poc_task")
    resources = relationship("Resource", back_populates="poc_task", cascade="all, delete-orphan")
...
class POCTaskGroup(Base):
    ...
    group_criteria = relationship("TaskSuccessCriteria", back_populates="poc_task_group")
    resources = relationship("Resource", back_populates="poc_task_group", cascade="all, delete-orphan")
