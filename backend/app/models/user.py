...
class User(Base):
    ...
    created_pocs = relationship(
        "POC", back_populates="created_by_user", foreign_keys="POC.created_by"
    )
    comments = relationship("Comment", back_populates="user")
    password_reset_tokens = relationship(
        "PasswordResetToken", back_populates="user"
    )
    created_public_links = relationship(
        "POCPublicLink",
    ...
    def has_role(self, role: "UserRole", tenant_id: int = None) -> bool:
        ...
        return any(
            tr.role == role and tr.tenant_id == tenant_id
