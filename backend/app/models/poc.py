"""POC model"""
from sqlalchemy import Column, Integer, String, Boolean, DateTime, ForeignKey, Text, Enum as SQLEnum, Date, Table
...
poc_products = Table(
    ...
    Column('poc_id', Integer, ForeignKey('pocs.id', ondelete='CASCADE'), nullable=False),
...
class POC(Base):
    ...
    created_by_user = relationship("User", back_populates="created_pocs", foreign_keys=[created_by])
    ...
    resources = relationship("Resource", back_populates="poc", cascade="all, delete-orphan")
    products = relationship("Product", secondary=poc_products, back_populates="pocs")
