...
@router.post("/", response_model=POCSchema, status_code=status.HTTP_201_CREATED)
def create_poc(
    poc_data: POCCreate,
...
def list_pocs(
    ...
    if customer_name:
        query = query.filter(POC.customer_company_name.ilike(f"%{customer_name}%"))
    
    # Filter by participant user name
    if user_name:
        query = query.join(POCParticipant).join(User).filter(
...
def get_poc(
    ...
    if not check_tenant_access(current_user, poc.tenant_id):
        ...
        participant = db.query(POCParticipant).filter(
    ...
    return {
        **poc.__dict__,
        "participants": [{"user_id": p.user_id, "is_sales_engineer": p.is_sales_engineer, "is_customer": p.is_customer} for p in participants],
...
async def add_participant(
    ...
    if participant_data.user_id:
        ...
        existing_user = db.query(User).filter(User.email == participant_data.email).first()
        if existing_user:
    ...
    existing = db.query(POCParticipant).filter(
        POCParticipant.poc_id == poc_id,
        POCParticipant.user_id == user_id
    ).first()
    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
def get_dashboard_stats(
    ...
    completed_pocs = query_base.filter(POC.status == POCStatus.COMPLETED).count()
    in_progress_pocs = query_base.filter(POC.status.in_([POCStatus.DRAFT, POCStatus.ACTIVE])).count()
...
def generate_poc_document(
    ...
    if not check_tenant_access(current_user, poc.tenant_id):
        ...
        is_participant = db.query(POCParticipant).filter(
            POCParticipant.poc_id == poc_id,
            POCParticipant.user_id == current_user.id
        ).first()
    ...
    safe_title = "".join(c for c in poc.title if c.isalnum() or c in (' ', '-', '_')).strip()
