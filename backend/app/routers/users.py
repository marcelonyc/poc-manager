...
def create_user(
    ...
    # Inherit tenant from current user if not specified
...
def update_user(
    ...
    # Check permissions
    if current_user.role != UserRole.PLATFORM_ADMIN:
        if user.tenant_id != current_user.tenant_id or user.id == current_user.id:
            if current_user.role not in [UserRole.TENANT_ADMIN, UserRole.ADMINISTRATOR]:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="Access denied",
                )
...
def deactivate_user(
    ...
    # Check access
    if current_user.role != UserRole.PLATFORM_ADMIN and user.tenant_id != current_user.tenant_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Access denied",
        )
