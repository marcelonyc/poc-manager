...
from app.auth import get_current_user, require_tenant_admin, get_current_tenant_id
...
async def create_tenant_invitation(
    ...
    existing_access = db.query(UserTenantRole).filter(
...
def list_tenant_invitations(
    ...
    invitations = query.order_by(TenantInvitation.created_at.desc()).offset(skip).limit(limit).all()
    
    result = []
    for inv in invitations:
        tenant_name = inv.tenant.name if inv.tenant else "Unknown"
        result.append(TenantInvitationResponse(
            id=inv.id,
            email=inv.email,
...
def validate_tenant_invitation(
    token: str,
    db: Session = Depends(get_db)
):
    """Validate a tenant invitation token (public endpoint for existing users)"""
    invitation = db.query(TenantInvitation).filter(TenantInvitation.token == token).first()
...
def accept_tenant_invitation(
    ...
    invitation = db.query(TenantInvitation).filter(
        TenantInvitation.token == accept_data.token
    ).first()
