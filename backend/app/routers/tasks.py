...
from app.schemas.task import (
    ...
    TaskGroupCreate, TaskGroupUpdate, TaskGroup as TaskGroupSchema,
    POCTaskCreate, POCTaskUpdate, POCTask as POCTaskSchema,
    POCTaskGroupCreate, POCTaskGroupUpdate, POCTaskGroup as POCTaskGroupSchema
)
from app.schemas.task_resource import (
    TaskResourceCreate, TaskResourceUpdate, TaskResource as TaskResourceSchema,
    TaskGroupResource as TaskGroupResourceSchema
)
from app.auth import require_administrator, require_sales_engineer, get_current_user, check_tenant_access
from app.utils.demo_limits import check_demo_task_limit, check_demo_task_group_limit

router = APIRouter(prefix="/tasks", tags=["Tasks"])


# Task Templates
@router.post("/templates", response_model=TaskSchema, status_code=status.HTTP_201_CREATED)
...
@router.post("/groups/templates", response_model=TaskGroupSchema, status_code=status.HTTP_201_CREATED)
def create_task_group_template(
    ...
    # Check demo limits
    check_demo_task_group_limit(db, current_user.tenant_id, current_user.tenant)
...
def add_task_to_group(
    ...
    if task in group.tasks:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Task already in group")
...
def remove_task_from_group(
    ...
    if task not in group.tasks:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Task not in group")
...
# POC Tasks
@router.post("/pocs/{poc_id}/tasks", response_model=POCTaskSchema, status_code=status.HTTP_201_CREATED)
def add_task_to_poc(
    poc_id: int,
...
def list_poc_tasks(
    ...
    tasks = db.query(POCTask).filter(POCTask.poc_id == poc_id).order_by(POCTask.sort_order).all()
    return tasks
...
def update_poc_task(
    ...
    """Update a POC task"""
    poc_task = db.query(POCTask).filter(POCTask.id == task_id, POCTask.poc_id == poc_id).first()
    if not poc_task:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Task not found")
...
# POC Task Groups
@router.post("/pocs/{poc_id}/task-groups", response_model=POCTaskGroupSchema, status_code=status.HTTP_201_CREATED)
...
@router.get("/pocs/{poc_id}/task-groups", response_model=List[POCTaskGroupSchema])
def list_poc_task_groups(
    ...
    groups = db.query(POCTaskGroup).filter(POCTaskGroup.poc_id == poc_id).order_by(POCTaskGroup.sort_order).all()
    return groups


@router.put("/pocs/{poc_id}/task-groups/{group_id}", response_model=POCTaskGroupSchema)
def update_poc_task_group(
    ...
    poc_group = db.query(POCTaskGroup).filter(POCTaskGroup.id == group_id, POCTaskGroup.poc_id == poc_id).first()
    if not poc_group:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Task group not found")
    ...
    if group_data.status == TaskStatus.COMPLETED and not poc_group.completed_at:
...
@router.post("/templates/{task_id}/resources", response_model=TaskResourceSchema, status_code=status.HTTP_201_CREATED)
...
@router.get("/templates/{task_id}/resources", response_model=List[TaskResourceSchema])
def list_task_template_resources(
    ...
    resources = db.query(TaskTemplateResource).filter(
        TaskTemplateResource.task_id == task_id
    ).order_by(TaskTemplateResource.sort_order).all()
    return resources


@router.put("/templates/{task_id}/resources/{resource_id}", response_model=TaskResourceSchema)
def update_task_template_resource(
    ...
    resource = db.query(TaskTemplateResource).filter(
...
@router.post("/groups/templates/{group_id}/resources", response_model=TaskGroupResourceSchema, status_code=status.HTTP_201_CREATED)
...
@router.get("/groups/templates/{group_id}/resources", response_model=List[TaskGroupResourceSchema])
def list_task_group_resources(
    ...
    resources = db.query(TaskGroupResource).filter(
        TaskGroupResource.task_group_id == group_id
    ).order_by(TaskGroupResource.sort_order).all()
    return resources


@router.put("/groups/templates/{group_id}/resources/{resource_id}", response_model=TaskGroupResourceSchema)
def update_task_group_resource(
    ...
    resource = db.query(TaskGroupResource).filter(
