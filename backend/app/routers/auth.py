...
def select_tenant(request: SelectTenantRequest, db: Session = Depends(get_db)):
    ...
    # Check if user is active in this tenant
    if not tenant_role.is_active:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Your access to this tenant has been deactivated",
        )

    # Verify tenant is active
    if (
        tenant_role.tenant_id
        and tenant_role.tenant
        and not tenant_role.tenant.is_active
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    access_token = create_access_token(
        data={"sub": user.email, "role": tenant_role.role.value},
